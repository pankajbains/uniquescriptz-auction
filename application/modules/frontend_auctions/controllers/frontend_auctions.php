...
*/
			...
			//$this->load->view('frontend_templates/index_v', $this->settings());
	...
	public function product($slug=NULL, $slugid=NULL)
	{
		...
		$data['content_user']=$this->frontend_auctions_m->get_users($user_id);
		...
		//$data['content_latest'] = $this->frontend_auctions_m->latestbids($slugid, $user_id);

		//$data['content_featured']=$this->frontend_auctions_m->latestbids($slugid);

		$this->frontend_templates->inner($data, $this->settings());
		
		//var_dump($data['content_bidunique_data']);
	}
	...
	{
		$data['content_data']=$this->frontend_auctions_m->get_details($auction);
	...
	public function place_now($slug=NULL)
	{
		...
		if($placebid[1]=='error'){
			...
			$user_name=ucwords($user_result[0]['first_name'].' '.$user_result[0]['last_name']);
