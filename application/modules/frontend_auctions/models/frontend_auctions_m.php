...
class frontend_auctions_m extends CI_Model {
		...
		public function get_auctiondetails($data){
			...
			$this->db->select('auction_items.auction_id,auction_items.auction_closed,auction_items.auction_open,auction_items.auction_bid,auction_items.auction_credits,auction_features.scurrent_bids,auction_items.auction_type,auction_items.auction_price,auction_items.auction_max_bid');
			$this->db->from('auction_items');
			...
			$auction_type = $q->result_array(); 
			$unique = ($auction_type[0]['auction_type'] == 'lowest')?min($single_bids):max($single_bids);
			$start_bids_price = $unique -  $randvalueneg;
			$end_bids_price = $unique + $randvalueplus;

			foreach($details as $dt) {
				$dt['start_bid'] = $start_bids_price;
				$dt['end_bid'] = $end_bids_price;
		...
		public function latestbids($auction, $user){
			...
			if($user!=0){
				...
				//$wharray = array('auction_bids.auction_id'=>$auction, 'user_register.user_id' => $user, 'bid_status !='=>'2');
			}else{
				$wharray = array('auction_bids.auction_id'=>$auction, 'bid_status !='=>'2');
			}
			$this->db->where($wharray);
...
--*/
		...
		public function post_now($data, $settings){
			...
			if(($user_auction_count>$auction_details[0]['auction_users_bid']) && ($auction_details[0]['auction_users_bid']!=0)){
			...
			// ---------------------- Prepare to Insert Bid on Database ------------------------//
					...
					if(count($auction_result)>0){
						...
						if($auction_format == 'lowest'){
							$wharray = array('bid_price < ' => $_POST['bid_price'], 'auction_id' => $_POST['auction_id']);
						}else if($auction_format == 'highest'){
							$wharray = array('bid_price > ' => $_POST['bid_price'], 'auction_id' => $_POST['auction_id']);
						}
					...
					if($auction_details[0]['auction_type']==1){
						...
						$this->db->set('paid_credit', 'paid_credit -'.$auction_details[0]['auction_credits'], FALSE);

					}else if($auction_details[0]['auction_type']==0){

						$this->db->set('free_credit', 'free_credit - '.$auction_details[0]['auction_credits'], FALSE);
					...
					//echo $mailtype.'-success-'.$bidsuccess;
