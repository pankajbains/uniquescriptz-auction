...
class frontend_users_auctions_m extends CI_Model {
		...
		public function get_user_auctions($open,$closed,$won=NULL){
			 ...
			 if($won!='0'){
				...
				$this->db->distinct();
				$this->db->select('auction_won.auction_id, auction_items.auction_name, auction_items.auction_edate, auction_items.auction_etime, auction_items.auction_bid, auction_items.auction_max_bid, auction_won.bid_price', false);
				$this->db->from('auction_won');
				$this->db->where($wharray);
				$this->db->join('auction_items', 'auction_won.auction_id = auction_items.auction_id');
			 	$this->db->join('auction_bids', 'auction_bids.auction_id = auction_items.auction_id');
				...
				$this->db->distinct();
				$this->db->select('auction_bids.auction_id, auction_items.auction_name, auction_items.auction_edate, auction_items.auction_etime, auction_items.auction_bid, auction_items.auction_max_bid, auction_bids.bid_price, auction_bids.bid_id', false);
		...
		public function get_winner_invoice($auctionid){
			...
			//$this->db->distinct();
			$this->db->select('auction_won.user_id,auction_invoice.status,auction_invoice.invoice_no, auction_won.auction_id,auction_items.auction_name,auction_items.auction_desc,auction_items.auction_nprice,auction_won.bid_price');
			$this->db->from('auction_won');
			$this->db->where($wharray);
			
			$this->db->join('auction_items', 'auction_won.auction_id = auction_items.auction_id');
			$this->db->join('auction_invoice', 'auction_won.auction_id = auction_invoice.auction_id');
			$query=$this->db->get();
			//var_dump($this->db->last_query());
